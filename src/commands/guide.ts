...
name: task-cycle
description: >
  plsr-taskmasterとai-meetingを連携し、plan/reviewサイクルを自動実行する。AIが自分の名前で実装・レビューを行い、交互にサイクルを回す。
---
...
meetingスキルと同じルールで自分の名前を決定する:
1. CLAUDE.mdの「ミーティングの際の名前」を確認
2. 未定義なら: Claude Code → \`claude-code\` / Codex → \`codex\`

run（実行）もreview（レビュー）も**同じ名前**でミーティングに投稿する。
...
8. **ミーティングで通知**
   - タスク名のスレッドが存在しなければ作成:
     \`\`\`bash
     mtg start "<taskName>" --name <agent> --message "plan.<taskName>.<N>.md の実行が完了しました。レビューをお願いします。"
     \`\`\`
   - スレッドが存在すれば追記:
     \`\`\`bash
     mtg say "plan.<taskName>.<N>.md の実行が完了しました。レビューをお願いします。" --name <agent>
     \`\`\`
...
7. **ミーティングで通知**
   \`\`\`bash
   mtg say "review.<taskName>.<N>.md のレビューが完了しました。改善をお願いします。" --name <agent>
   \`\`\`
...
ユーザー: ptm create myTask → plan.myTask.1.md 作成
    ↓
AI: /task-cycle run → 実装 → review.myTask.1.md 作成 → ミーティング通知
    ↓
AI: /task-cycle review → 所見記入 → plan.myTask.2.md 作成 → ミーティング通知
    ↓
AI: /task-cycle run → 改善 → review.myTask.2.md 作成 → ミーティング通知
    ↓
（繰り返し）
    ↓
ユーザー: ptm done → アーカイブ
