...
作成したファイルのフルパスは、そのままAIへ渡せるようクリップボードへコピーされます。
...
生成したファイルのフルパスはクリップボードへコピーされます（既に存在していた場合も、そのファイルのパスをコピーします）。
...
- 復元に成功すると、そのタスクの**最新タスクファイル**（サイクル番号が最大のもの。同番号ならreview）のフルパスがクリップボードへコピーされます。
